...
const Request = () => {
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            <h1 className="text-3xl font-bold text-center mb-6 text-gray-700">Connections Request</h1>
            <div className="flex flex-col items-center space-y-6">
                {request.map((requests) => {
                    const { firstName, lastName, age, gender, bio, imageUrl, _id } = requests.fromUserId;
                    return (
                        <div key={_id} className="w-4/5 bg-gray-200 shadow-md rounded-xl p-5 flex items-center space-x-6 hover:shadow-lg transition-shadow duration-300">
                            {/* Profile Image */}
                            <img
                                src={imageUrl}
                                alt={firstName}
                            ...
                            <div className="flex-1">
                                {/* Name */}
                                <h2 className="text-2xl font-semibold text-gray-700">{firstName} {lastName}</h2>

                                {/* Age & Gender */}
                                <p className="text-gray-700 mt-1 text-lg font-medium">{gender}, {age} years old</p>
                            ...
                            <button className="mt-4 px-4 py-2 bg-gray-500 text-white text-sm font-medium rounded-lg hover:bg-gray-600 transition duration-200 cursor-pointer"
